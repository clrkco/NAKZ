...
class FaceBoundsOverlay @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        facesBounds.forEach {
            ...
            centerX = computeFaceBoundsCenterX(canvas.width.toFloat(), scaleX(it.box.exactCenterX(), canvas))
            centerY = computeFaceBoundsCenterY(canvas.height.toFloat(), scaleY(it.box.exactCenterY(), canvas))
    ...
    private fun computeFaceBoundsCenterX(viewWidth: Float, scaledCenterX: Float) = when {
        ...
        cameraFacing == Facing.FRONT && cameraOrientation == Orientation.ANGLE_180 -> scaledCenterX
        else -> viewWidth - scaledCenterX
    }
    ...
    private fun computeFaceBoundsCenterY(viewHeight: Float, scaledCenterY: Float) = when {
        ...
        cameraFacing == Facing.FRONT && cameraOrientation == Orientation.ANGLE_180 -> viewHeight - scaledCenterY
        else -> scaledCenterY
    }
    ...
    private fun drawBounds(box: Rect, canvas: Canvas, centerX: Float, centerY: Float) {
        xOffset = scaleX(box.width() / 2.0f, canvas)
        yOffset = scaleY(box.height() / 2.0f, canvas)
